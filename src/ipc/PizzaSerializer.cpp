...
PackedPizza pack(const Pizza& pizza) {
  return {.bytes = {static_cast<char>(pizza.type),
                    static_cast<char>(pizza.size)}};
}

Pizza unpack(const PackedPizza& data) {
  return {.type = static_cast<PizzaType>(data.bytes[0]),
          .size = static_cast<PizzaSize>(data.bytes[1])};
}
